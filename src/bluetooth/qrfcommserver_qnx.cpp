...
void QRfcommServerPrivate::controlEvent(ppsResult result)
{
    if (result.msg == QStringLiteral("service_connected")) {
        ...
        if (result.dat.contains("addr") && result.dat.contains("uuid") && result.dat.contains("subtype")) {
            ...
            QString uuid = result.dat.at(result.dat.indexOf("uuid") + 1);
            int subtype = result.dat.at(result.dat.indexOf("subtype") + 1).toInt();
            qBBBluetoothDebug() << "Getting mount point path" << uuid << nextClientAddress<< subtype;
            ppsSendControlMessage("get_mount_point_path", 0x1101, QBluetoothUuid(uuid), nextClientAddress, this, BT_SPP_SERVER_SUBTYPE);
        } else {

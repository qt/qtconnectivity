...
const char btControlFDPath[] = "/pps/services/bluetooth/public/control";
const char btSettingsFDPath[] = "/pps/services/bluetooth/settings";
const char btRemoteDevFDPath[] = "/pps/services/bluetooth/remote_devices/";

const int ppsBufferSize = 1024;
...
void ppsRegisterControl ()
{
    ...
    if (count == 0) {
        ppsCtrlFD = open (btControlFDPath, O_RDWR | O_NONBLOCK);
        if (ppsCtrlFD == -1) {
            qWarning() << Q_FUNC_INFO << "ppsCtrlFD - failed to open" << btControlFDPath;
        }
        else {
...
pps_encoder_t *beginCtrlMessage(const char* msg, QObject *sender)
{
    ...
    pps_encoder_initialize( encoder, NULL);
...
void ppsSendControlMessage(const char* msg, int service, QBluetoothUuid uuid, QString address, QObject* sender)
{
...
void ppsSendControlMessage(const char* msg,  QString dat, QObject* sender)
{
    pps_encoder_t *encoder = beginCtrlMessage(msg, sender);
    ...
    if (ppsCtrlFD != -1) {
        ...
        if (pps_decoder_parse_pps_str(&ppsDecoder, buf) == PPS_DECODER_OK) {
            ...
            if (nodeType == PPS_TYPE_STRING) {
                ...
                do {
                    if (pps_decoder_get_string(&ppsDecoder,NULL, &buf) == PPS_DECODER_OK) {
...
QVariant ppsReadSetting(const char *property)
{
    int settingsFD;
    char buf[ppsBufferSize];
    if ((settingsFD = open(btSettingsFDPath, O_RDONLY)) == -1) {
    ...
    read( settingsFD, &buf, sizeof(buf) );
...
QVariant ppsRemoteDeviceStatus(QByteArray address, const char *property)
{
    int rmFD;
    char buf[ppsBufferSize];
    ...
    if ((rmFD = open(filename.constData(), O_RDONLY)) < 0) {
...
bool ppsReadRemoteDevice(int fd, pps_decoder_t *decoder, QBluetoothAddress *btAddr, QString *deviceName)
{
    ...
    if (read(fd, &buf, sizeof(buf)) == -1) {
        qWarning() << Q_FUNC_INFO << "Could not read from pps remote device file";
        return false;
    }

    qBBBluetoothDebug() << "Remote device" << buf;

    //the address of the BT device is stored at the beginning of the read
    if (buf[0] != '-') {
...
void ppsRegisterForEvent(QString evt, QObject *obj)
{

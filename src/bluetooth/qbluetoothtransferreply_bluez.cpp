...
NokiaBtServiceConnection *NokiaBtServiceConnection::s_instance = NULL;
#endif
...
#ifdef NOKIA_BT_SERVICES
    NokiaBtServiceConnection::instance()->acquire();
#endif
}
...
#ifdef NOKIA_BT_SERVICES
    NokiaBtServiceConnection::instance()->setTransferFinished(in0.path(), true);
#endif
}
...
#ifdef NOKIA_BT_SERVICES
    NokiaBtServiceConnection::instance()->setTransferFinished(in0.path(), false);
#endif
}
...
#ifdef NOKIA_BT_SERVICES
    NokiaBtServiceConnection::instance()->setTranferProgress(in0.path(), in1, m_size);
#endif
}
...
#ifdef NOKIA_BT_SERVICES
    QFile *file = qobject_cast<QFile *>(source);
    NokiaBtServiceConnection::instance()->outgoingFile(m_transfer_path, address.toString(), file->fileName(), QBluetoothTransferReply::attribute(QBluetoothTransferRequest::TypeAttribute).toString(), m_size);
    NokiaBtServiceConnection::instance()->setTransferStarted(m_transfer_path);
#endif
...
#ifdef NOKIA_BT_SERVICES
        NokiaBtServiceConnection::instance()->setTransferFinished(m_transfer_path, false);
#endif
...
NokiaBtServiceConnection* NokiaBtServiceConnection::instance()
{
    if (s_instance == NULL) {
        s_instance = new NokiaBtServiceConnection();
    }
    return s_instance;
}

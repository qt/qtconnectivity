...
//        if (BPS_SUCCESS != navigator_invoke_set_filters("20", "org.qtm.NFCTest",
//                                        0, 0)) {
//            qWarning() << Q_FUNC_INFO << "Could not reset the invokation filters";
//        }
//    }
    qDebug() << "Setting up invoke target";
    QByteArray mimeFilter = "types=application/vnd.rim.nfc.ndef;";
    QByteArray uriFilter = "uris=ndef://1/U;";
    QByteArray theFilter = "actions=bb.action.OPEN;types=application/vnd.rim.nfc.ndef; + uriFilter;uris=ndef://1/U;";
...
//    }
    ...
    filters[0] = theFilter.constData();
//    filters[0] = QByteArray("actions=bb.action.OPEN;").data();
//    filters[1] = QByteArray("types="+mimeFilter).data();
//    if (!registerAll)
//        filters[2] = uriFilter.data();

    if (BPS_SUCCESS != navigator_invoke_set_filters("20", "org.qtm.NFCTest",
                                    filters, 1)) {
        qWarning() << "NFC Error setting share target filter";
    } else {
        qDebug() << "NFC share target filter set";
    }
}

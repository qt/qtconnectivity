...
bool QNXNFCEventFilter::nativeEventFilter(const QByteArray &eventType, void *message, long *result)
{
    ...
    if (code == NAVIGATOR_INVOKE_TARGET) {
        ...
        qDebug() << "--------Got Invoke event" << uri << "Type" << type;
        qDebug() << message.size();
        qDebug() << ndefRecordCount;
        qDebug() << message.at(0).type();
        qDebug() << "Data" << dataLength << data;
    }

    return false;//prevFilter->nativeEventFilter(eventType, message, result);
}
